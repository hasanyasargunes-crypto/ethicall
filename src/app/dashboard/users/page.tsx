...
export default function UsersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/organizations", {
      ...
      body: JSON.stringify({ orgName, slug, domain, emailDomain, plan, billingPeriod, adminName, adminEmail, adminPassword }),
    });
            ...
            <tbody>
              {orgs.map((org) => (
                <tr key={org.id}>
                  ...
                  <td>
                    <span className="flex items-center gap-1 text-[13px] text-gray-600"><FileText className="h-3.5 w-3.5" />{org._count.reports}</span>
                  </td>
                  <td>
